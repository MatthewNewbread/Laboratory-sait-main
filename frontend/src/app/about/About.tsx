  ...
  return (
    <section className="relative w-full bg-black pb-[clamp(208px,17vw,400px)] z-[1]">
      ...
      <div className="overflow-hidden w-full mt-[clamp(14px,calc(-10px+3.13vw),35px)]">
        <div className="flex animate-slider-scroll w-max" style={{ animationDuration: `${scrollSpeed}ms` }}>
          {[...images, ...images].map((src, idx) => (
            <div key={idx} className="flex-shrink-0 w-[clamp(266px,calc(0.66px+34.55vw),664px)] mr-[clamp(10px,1.57vw,30px)] overflow-hidden rounded-[10px]">
              <Image
                src={src}
                alt={`slide-${idx}`}
                width={664}
      ...
      <div id="about" className="flex items-center justify-end w-full py-13 scroll-mt-[45px]">
        <div className={`relative w-[274px] h-[76px] transition-transform duration-500 ease-out origin-bottom-right ${hovered ? "scale-100 opacity-100" : "scale-0 opacity-0"}`}>
          <Image
      ...
      <div className="w-full text-white px-[clamp(40px,calc(-137px+23.07vw),195px)] whitespace-pre-line">
        <div className="mx-auto max-w-[1530px] flex flex-col gap-15">
          ...
          <div className="flex items-start gap-[20px]">
            <div lang="ru" className="relative w-[clamp(490px,calc(58.12px+68.73vw),1377px)] text-writing text-primary leading-snug hyphens-auto break-words">
              {text}
          ...
          <div className="flex flex-col xl:flex-row items-center text-primary text-main gap-[30px] xl:gap-[clamp(30px,2vw,78px)] ">
            <div className="flex flex-col xl:flex-row items-center gap-[7px] shrink-0">
              <p className="text-[clamp(80px,calc(359px-27.3vw),150px)] font-medium">3</p>
